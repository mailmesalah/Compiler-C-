	...
	try {
		...
		for (vector<string> n : instructionSet) {
			if (strcmp(n[0].c_str(), "jumpl") == 0 && count < jump.size()) {
				outputFile << "n" << jump[count] << "-> n" << (atoi(n[3].substr(1, n[3].length()).c_str()) - 1) << endl;
				count++;
			}
			else if (strcmp(n[0].c_str(), "cbr") == 0 && j < cbr.size()) {
				outputFile << "n" << cbr[j] << "-> n" << (atoi(n[3].substr(1, 2).c_str()) - 1) << endl;
				outputFile << "n" << cbr[j] << "-> n" << (atoi(n[3].substr(4, 5).c_str()) - 1) << endl;
				j++;
			}
		}
		outputFile << endl;
