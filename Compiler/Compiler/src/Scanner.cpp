...
std::list<string> Scanner::analyse(string inputFile) {
		...
		while (getline(inFile, line))
		{
			if (!line.empty()) {
				...
				if (line.find("%") != std::string::npos) {
					currentLine = line.substr(0, line.find('%'));
				...
				//tok= strtok_s(str, &strmax, " ", &next_token);
				tok = strtok_s(str, " ", &next_token);
